...
export const ABOUT: Record<Language, LegalDoc> = {
  ka: {
    ...
    sections: [
      {
        title: "ვინ ვართ",
        body: `Cheapers.ge არის პლატფორმა, რომელიც ყოველდღიურად გაძლევთ შესაძლებლობას აღმოაჩინოთ საუკეთესო ფასდაკლებები რესტორნებში, კაფეებში, საცხობებსა და სხვა კვების ობიექტებში.

ჩვენი მიზანია, ხარისხიანი საკვები ყველასთვის უფრო ხელმისაწვდომი გავხადოთ. Cheapers.ge-ზე ერთ სივრცეში იპოვით მრავალფეროვან ყოველდღიურ შეთავაზებებს, სპეციალურ ფასებსა და ექსკლუზიურ შეთავაზებებს, რომლებიც დაგეხმარებათ ისიამოვნოთ თქვენი საყვარელი კერძებით უფრო ხელსაყრელ ფასად.
...
Email: ${COMPANY.email}`,
  ...
  ru: {
    ...
    sections: [
      {
        title: "Кто мы",
        body: `Cheapers.ge — это платформа, которая каждый день помогает вам находить лучшие скидки в ресторанах, кафе, пекарнях и других заведениях питания.

Наша цель — сделать качественную еду доступной для каждого. На Cheapers.ge в одном месте вы найдёте разнообразные ежедневные предложения, специальные цены и эксклюзивные акции, которые позволят наслаждаться любимыми блюдами по более выгодной цене.
...
export const PRIVACY: Record<Language, LegalDoc> = {
  ka: {
    ...
    subtitle: `${COMPANY.nameKa} · cheapers.ge`,
    back: "უკან",
    sections: [
      {
        title: "შესავალი",
        body: `წინამდებარე ვებგვერდი cheapers.ge (შემდგომში „ვებგვერდი") არის ${COMPANY.nameKa}-ს (ს/კ ${COMPANY.id}) (შემდგომში „ჩვენ") საკუთრება.
...
მისამართი: საქართველო, თბილისი, ნ. ვაჩნაძის №7, 0105`,
  ...
  en: {
    ...
    subtitle: `${COMPANY.nameEn} · cheapers.ge`,
    back: "Back",
    sections: [
      {
        title: "Introduction",
        body: `This website cheapers.ge (the "Website") is owned by ${COMPANY.nameEn} (Company ID ${COMPANY.id}) (the "Company", "we").
...
Address: 7 N. Vachnadze Str., Tbilisi 0105, Georgia`,
  ...
  ru: {
    ...
    subtitle: `${COMPANY.nameRu} · cheapers.ge`,
    back: "Назад",
    sections: [
      {
        title: "Введение",
        body: `Настоящий сайт cheapers.ge (далее — «Сайт») принадлежит ${COMPANY.nameRu} (идентификационный код ${COMPANY.id}) (далее — «мы»).
