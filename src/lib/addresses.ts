...
export function formatAddressDetails(
  ...
  const parts: string[] = [];
  if (a.entrance) parts.push(`${L("სადარბაზო", "entrance", "подъезд")} ${a.entrance}`);
  if (a.floor) parts.push(`${L("სართული", "floor", "этаж")} ${a.floor}`);
  if (a.apartment) parts.push(`${L("ბინა", "apt", "кв.")} ${a.apartment}`);
  if (a.door_code) parts.push(`${L("კოდი", "code", "код")} ${a.door_code}`);
  return parts.join(" · ");
}

export function addressLabelText(a: UserAddress, language: "ka" | "en" | "ru"): string {
  if (a.custom_label) return a.custom_label;
  const L = (ka: string, en: string, ru: string) =>
    language === "en" ? en : language === "ru" ? ru : ka;
  if (a.label === "home") return L("სახლი", "Home", "Дом");
  if (a.label === "work") return L("სამსახური", "Work", "Работа");
  return L("სხვა", "Other", "Другое");
}
